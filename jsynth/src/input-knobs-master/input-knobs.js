  ...
  let InitKnobs = (el) => {
    ...
    ik.pointerdown = (ev) => {
      ...
      if (ik.itype != "k" && op.sliderMode == "abs") {
        let dv = (ik.valrange.min + ik.valrange.max) * 0.5 + ((dx - cx) / ik.sensex - (dy - cy) / ik.sensey) * (ik.valrange.max - ik.valrange.min);
        el.setValue(dv);
      ...
      //this works on the build version for mobile, but not the test version 
      ev.preventDefault();
      ev.stopPropagation();
    };
    ik.pointermove = (ev) => {
